...
def save_metrics(checkpoint_path_metrics, epoch, phase, loss, accuracy, f1Score, precision, recall):
    ...
    epoch_metrics = {'Epoch': epoch,
                     'Phase': phase,
                     'Loss': loss.deatch().cpu().clone().numpy(),
                     'Accuracy': accuracy.deatch().cpu().clone().numpy(),
                     'F1Score': f1Score.deatch().cpu().clone().numpy(),
                     'Precision': precision.deatch().cpu().clone().numpy(),
                     'Recall': recall.deatch().cpu().clone().numpy()
                     }
