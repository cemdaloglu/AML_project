...
if __name__ == '__main__':
    ...
    parser.add_argument('-best', '--best_model', help='The model which had the highest accuracy to show the entire city prediction', default = "unet_lr001_bs128_cel_2layer", type=str)
    parser.add_argument('-m_list', '--model_list', help='List of length 4 with the 2 layer U-Net model, the 4-layer U-Net model, VGG16 pretrained model and VGG16 index model', nargs="*" , type=str, action='store', dest='model_list', default = ["unet_CEL_lr001_hflip_vflip03_wd01", "unet_CEL_lr001_hflip_vflip03_wd01", "unet_CEL_lr001_hflip_vflip03_wd01", "unet_CEL_lr001_hflip_vflip03_wd01"])
    parser.add_argument('-patch_dir', '--patch_test_dir', help='Directory where the groundtruth patched masks and images lie of the test set.', default = "patches/test")
    ...
    plot_worst_segmentations(args.patch_test_dir, args.result_path, args.best_model)
